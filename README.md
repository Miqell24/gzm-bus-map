...
## 18.09.2026 — requested fixes

- **The metrolines' amber is gone from the street rows too.** On 17.09 the strokes, the legend and the panel turned navy, but the number rows beside the streets still carried their own `mLines` group and the front end kept painting it amber. The group no longer exists (`build.mjs`), the M numbers ride at the head of the navy group, and the whole independent-category mechanism (toggle, dashed overlay, amber icons) left `app.js`.
- **The S-lines in the rapid-rail convention, drawn like Berlin's S-Bahn.** Koleje Śląskie take the family's metro treatment (`allMetro`): one full disc per station and station names that are never dropped, one translucent ribbon per stretch of track. Where a single line runs, the ribbon wears that line's colour from the operator's *Schemat linii komunikacyjnych*; where several share the track it is the operator purple; a picked line repaints its ribbon in its own colour end to end (`RAIL_COLOR` in app.js). A first attempt drew shared track as parallel stripes, one per colour — rejected by the user the same day ("zrób to w tym samym stylu co S-Bahn w Berlinie"); what it left behind is the clean axis below.
- **The railway on one clean axis per corridor** (`pipeline/railaxis.mjs`). The first cut of the stripes showed every flaw of the matched geometry at once: a double-track line is two OSM ways, one direction rides each and the path hops across at every crossover, so the bundle split in two with different colours in each half; at stations the path wandered into a loop or a siding and back; every run had its own direction, so a wide ribbon zig-zagged. The pass rebuilds the railway from the matched line-directions: out-and-back excursions are cut (also the ones that come back on the parallel track, heading the other way) unless one of the line's own stations is out there — the trains really do reverse at Rybnik, Chybie, Goleszów and Bielsko-Biała Gł.; routes are laid down longest first and ride an existing axis wherever they run within 55 m of it, leaving it where the rails part (peeled back to 12 m); stations move onto the axis; the rail number rows are regenerated, one per stretch with one set of lines. 112 line-directions → 34 axes → 77 stretches, 1 057 km.
- **Feed shapes that wander between two stations** (`detourFix` on the Koleje Śląskie cfg). The operator's shape for S51 Zakopane – Częstochowa runs from Sucha Beskidzka down to Żywiec and Bielsko-Biała, then out to Kalwaria and back: 121 km between two stations 15 km apart, faithfully followed by the matcher (the line measured 411 km). Where the shape between two consecutive stops is longer than 2.5 × the straight line + 3 km its vertices are dropped and the router finds the track: six stretches (S51, S6 ×4, S61); S51 is 306 km again and runs via Kalwaria and Wadowice, as the train does.
