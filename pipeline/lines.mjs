...
// Koleje Śląskie S-lines keep the operator's own colours here too (user
// 17.09.2026: rail drawn in the metro convention, colours from the KŚ line
// map). They are pinned BEFORE the palette runs, so the buses sharing their
// bundles are pushed away from them, never the other way round.
const hex2lab = (h) => {
...
for (const l of meta.lines) {
  if (!l.rail || !/^#[0-9a-f]{6}$/i.test(l.color || '') || l.color.toLowerCase() === '#a518a3') continue;
