...
// as in GZM — all modes read ONE feed and are separated by route_type.
...
// m — longer jumps between shape points are GTFS data gaps. The ZTM GZM feed
// samples shapes at ~51 m median (p90 194 m, p99 492 m, holes to 1.5 km), so the
...
async function processMode(cfg) {
  ...
  // direction_id alone can lie (the Athens/OSY defect): 8 lines here (8, 155,
  // 196, 255, 674, 676, 813, G) carry one constant value on ALL their trips
  // while the return direction exists as separate trips with a mirrored
  // headsign — grouped by direction_id alone both directions collapse into
  // one bucket and only the busier one gets drawn. Pre-scan direction
