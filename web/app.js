...
async function init() {
    ...
    document.getElementById('chips').innerHTML = CATS.map(([title, test]) => {
      ...
      return `<span class="chip-cat">${title}</span>` + ls
        .map((l) => `<button class="chip${l.line === state.selected ? ' active' : ''}" data-line="${esc(l.line)}" ` +
          `style="background:${esc(linesView ? lineColor(l.line) : l.color)}">${esc(dispLine(l.line))}</button>`)
        .join(' ');
    }).join('');
  };
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      a.download = `gzm-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `gzm-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // conurbation as a stripe. Open on the Katowice–Sosnowiec core instead.
  map.jumpTo({ center: [19.02, 50.255], zoom: 10.6 });
}
